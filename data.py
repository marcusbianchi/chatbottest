...
def question_answers():
    ...
    c = connection.cursor()
    sql = "SELECT y.comment as question,x.comment as answer FROM parent_reply as x inner join parent_reply as y on x.parent_id = y.comment_id"
    c.execute(
        "select count(*) FROM parent_reply as x inner join parent_reply as y on x.parent_id=y.comment_id")
...
def prepare_dataset(questions, answers):
    ...
    test_ids = random.sample(
        [i for i in range(len(questions))], config.TESTSET_SIZE)
...
def get_batch(data_bucket, bucket_id, batch_size=1):
    ...
    batch_encoder_inputs = _reshape_batch(
        encoder_inputs, encoder_size, batch_size)
